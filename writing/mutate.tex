...
The astute reader must have observed that mutators that cause interesting
...
benchmarks are fully typed, and type checking flags each impedance at
compile time.  Hence, we apply the mutators to versions, dubbed
\dubbed{configurations}, of the benchmarks where some components are
stripped of their type annotation.  Without the annotations, the type
checker may not detect an impedance and, instead, the type-level checks
of the gradual type system signal a run time error.  Mutant configurations
...
mutant configuration that
\begin{enumerate} 
  \item is rejected by the type checker when we restore any missing type annotations, 
