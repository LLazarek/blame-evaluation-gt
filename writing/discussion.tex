...
Unfortunately, our high cost is closer to the truth;
 Reticulated has two advantages that give low overhead in the prior work.
First, Reticulated's type inference often assigns the dynamic type to local
 variables in "fully typed" code.
This flexible type gives a programmer fewer static errors and less-precise
 blame,; but it also makes the benchmarks run faster.
Second, \citet{vss-popl-2017} use relatively small benchmarks.
Four have since been retired from the official Python benchmark suite
 because they are too small, unrealistic, and unstable.
