...
worst-case of 17.2x on the fully-typed Python benchmarks in Reticulated
Python with the blame map enabled.  Unfortunately, the orders-of-magnitude
higher cost of Shallow Racket seems closer to the truth.  There are at
least four broad issues that skew the earlier results:
\begin{enumerate}
...
\item While Reticulated attempts to infer types for local variables,
  often local variables end up with type
{\tt Dynamic} due the impoverished nature of the type system,  which does
    not allow the ascription of precise
    types~\citet{g-dissertation-2020}. Code with the type {\tt
...
\item Reticulated Python does not seem to employ any crucial
  implementation techniques that can mitigate the significant slowdown due to
    the blame map observed in Shallow Racket. For example Reticulated Python  runs the simplest
    benchmark from the GTP suite (translated into Python) in approximately
    40 seconds without blame but times out after 10 minutes with blame.
